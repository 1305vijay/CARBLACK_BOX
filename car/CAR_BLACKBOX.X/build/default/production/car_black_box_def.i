...
unsigned char login(unsigned char key, unsigned char reset_flag)
{
    ...
    if(i == 4)
    {
        ...
        if(!strncmp(user_pass, save_pass,4))
        {
            ...
            clcd_print("Login Success", (0x80 + 1));
        ...
        else
        {
            ...
            else
            {
                ...
                clcd_putch(attempts_left + '0', (0xC0 + 14));
                _delay((unsigned long)((3000)*(20000000/4000.0)));
            }
            clear_display();
...
unsigned char view_log(unsigned char key, unsigned char start_flag)
{
    ...
    {
        clear_display();
        clcd_print("NO LOGS PRESENT", (0xC0 + 0));
    ...
    else if (key == 0x2F && pos < event_count - 1 )
    {
        pos++;
        clear_display();
    }
    ...
    {
        addrs = 0x05 + ((latest_log + pos) % 10) * 10;
    }
    ...
    else
    {
        ...
        clcd_putch(logs[0], (0xC0 + 2));
        clcd_putch(logs[1], (0xC0 + 3));
        clcd_putch(':', (0xC0 + 4));
        clcd_putch(logs[2], (0xC0 + 5));
        clcd_putch(logs[3], (0xC0 + 6));
        clcd_putch(':', (0xC0 + 7));
        clcd_putch(logs[4], (0xC0 + 8));
        clcd_putch(logs[5], (0xC0 + 9));

        clcd_putch(logs[6], (0xC0 + 11));
        clcd_putch(logs[7], (0xC0 + 12));

        clcd_putch(logs[8], (0xC0 + 14));
        clcd_putch(logs[9], (0xC0 + 15));
...
unsigned char clear_log(unsigned char start_flag)
{
    ...
    if(start_flag == 'B' && log_flag)
    {
        ...
        clcd_print("LOGS  ARE",(0x80 + 4));
        clcd_print("CLEARED",(0xC0 + 5));
    }
    else if(!log_flag)
    {
        clcd_print("NO LOGS ", (0x80 + 4));
        clcd_print("PRESENT", (0xC0 + 4));
    }
...
unsigned char download_log(unsigned char start_flag)
{
    ...
    if(!log_flag)
    {
        clcd_print("FOUND NO LOGS",(0xC0 + 1));
        puts("FOUND NO LOGS");
    }
    ...
    else if(start_flag == 'D' && log_flag)
    {
        ...
        clcd_print("OPEN YOUR",(0x80 + 3));
        clcd_print("CUTECOM/TERATERM",(0xC0 + 0));
        puts("LOGGED DATA");
        putchar('\n');putchar('\r');
...
unsigned char set_time_log(unsigned char key, unsigned char start_flag)
{
    ...
    static unsigned char blink = 1, once = 1;
    ...
    if(timer++ == 2)
    {
        ...
        if(blink)
        {
            ...
            {
                clcd_print("  ",(0xC0 + 3));_delay((unsigned long)((150)*(20000000/4000.0)));
            }
            else if(cur_pos == positions[1])
            {
                clcd_print("  ", (0xC0 + 6));_delay((unsigned long)((150)*(20000000/4000.0)));
            }
            else if(cur_pos == positions[2])
            {
                clcd_print("  ", (0xC0 + 9));_delay((unsigned long)((150)*(20000000/4000.0)));
            }
        }
    }
    ...
    clcd_print("HH MM SS", (0x80 + 3));


    clcd_putch(new_time[0]/10 + '0', (0xC0 + 3));
    clcd_putch(new_time[0]%10 + '0', (0xC0 + 4));
    clcd_putch(':', (0xC0 + 5));
    clcd_putch(new_time[1]/10 + '0', (0xC0 + 6));
    clcd_putch(new_time[1]%10 + '0', (0xC0 + 7));
    clcd_putch(':', (0xC0 + 8));
    clcd_putch(new_time[2]/10 + '0', (0xC0 + 9));
    clcd_putch(new_time[2]%10 + '0', (0xC0 + 10));
...
unsigned char change_passwd(unsigned char key, unsigned char start_flag)
{
    ...
    char save_pass[4], compare_flag = 0;
    ...
    {
        if(toggle)
            clcd_putch('_',(0xC0 + 4+index));
        else if(!toggle)
            clcd_putch(' ',(0xC0 + 4+index));
    }

    if (re_enter)
    {
        clcd_print("ENTER NEW PASSWD", (0x80 + 0));
    ...
    else
    {
        ...
        clcd_print("RE-ENTER PASSWD", (0x80 + 0));
    ...
    if (compare_flag)
    {
        if (!strcmp(new_pass, re_new_pass))
        {
            ...
            clcd_print("PASSWORD CHANGED", (0xC0 + 0));
            ext_eeprom_24C02_str_write(0x00, new_pass);
        }
        else
        {
            clcd_print("FAILURE PASSWORD", (0x80 + 0));
            clcd_print("NOT MATCHING", (0xC0 + 2));
        }
